...
process.on('message', async (message) => {
  ...
  try {
    ...
    if (message.action === 'refreshProducts') {
      if (running) {
        send(requestId, { type: 'error', error: 'amazon product worker busy' });
        return;
      }
      running = true;
    ...
    running = false;
    send(requestId, { type: 'error', error: error?.message || 'amazon product worker failed' });
  }
});
