/**
 * Flipkart product worker process
 */
...
process.on('message', async (message) => {
  ...
  try {
    ...
    if (message.action === 'refreshProducts') {
      if (running) {
        send(requestId, { type: 'error', error: 'flipkart product worker busy' });
        return;
      }
      running = true;
    ...
    running = false;
    send(requestId, { type: 'error', error: error?.message || 'flipkart product worker failed' });
  }
});

process.on('disconnect', () => {
  void closePersistentBrowsers();
});
