...
process.on('message', async (message) => {
  ...
  try {
    ...
    if (message.action === 'refreshProducts') {
      if (running) {
        send(requestId, { type: 'error', error: 'myntra product worker busy' });
        return;
      }
      running = true;
    ...
    running = false;
    send(requestId, { type: 'error', error: error?.message || 'myntra product worker failed' });
  }
});
